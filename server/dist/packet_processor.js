        ...
        (0, child_process_1.exec)(tsharkCommand, { maxBuffer: maxBuffer }, async (error, stdout, stderr) => {
            ...
            try {
                ...
                packetsArray.forEach((packetObj) => {
                    ...
                    if (!assignedStream) {
                        // Create a new stream
                        let newStream = Create_Stream(packet.connectionID, packet.SourceIP, packet.DestinationIP, packet.ActivationID, packet.Protocol, true, packet.Timestamp, packet.Timestamp, 0, 0, BigInt(0), packet.InterfaceAndProtocol);
...
function fromWiresharkToPacketObject(packetObj) {
    ...
    if (tcp) {
        ...
        if (sourcePort == 443 || DestPort == 443) {
            InterfaceAndProtocol = 'HTTPS';
        }
        else if (sourcePort == 80 || DestPort == 80) {
            InterfaceAndProtocol = 'HTTP';
        }
        else if (sourcePort == 22 || DestPort == 22) {
            InterfaceAndProtocol = 'SSH';
        }
        else if (sourcePort == 53 || DestPort == 53) {
            InterfaceAndProtocol = 'DNS';
        }
        else {
            InterfaceAndProtocol = 'OTHER';
        }
    }
    if (udp) {
    ...
    Timestamp, Size, (main_1.currentactivation === null || main_1.currentactivation === void 0 ? void 0 : main_1.currentactivation.ActivationID) || 0, sourceMAC, destinationMAC, sourcePort, DestPort, flags, frameLength, connectionID, InterfaceAndProtocol, tcpFlags, tcpSeq, tcpAck, tcpChecksumStatus, udpChecksumStatus, arpOpcode, ipChecksumStatus, false, // errorIndicator, initially false
